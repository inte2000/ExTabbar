...
public:
	...
	void CancelItemDrag(bool bRedrawEffectedArea = true)
	{
		...
		NMCTCITEM nmh = {{ m_hWnd, (UINT_PTR)this->GetDlgCtrlID(), CTCN_CANCELITEMDRAG }, m_iDragItemOriginal, {-1,-1}, 0};
		::SendMessage(GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM)&nmh);
...
public:
	...
	LRESULT OnLButtonDown(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/)
	{
		...
		else
		{
			...
			NMCTCITEM nmh = {{ m_hWnd, (UINT_PTR)this->GetDlgCtrlID(), NM_CLICK }, nIndex, {ptCursor.x, ptCursor.y}, tchti.flags};
			if(FALSE == ::SendMessage(GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM)&nmh))
			{
				// returning FALSE let's us do our default handling
				if(nIndex >= 0)
	...
	LRESULT OnLButtonUp(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/)
	{
		if(m_hWnd == ::GetCapture())
		{
			...
			{
				// Close Button
				NMCTCITEM nmh = {{ m_hWnd, (UINT_PTR)this->GetDlgCtrlID(), CTCN_CLOSE }, m_iCurSel, {ptCursor.x, ptCursor.y}, 0};
				::SendMessage(GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM)&nmh);
			}
			...
			{
				// New Tab Button
				NMCTCITEM nmh = {{ m_hWnd, (UINT_PTR)this->GetDlgCtrlID(), CTCN_NEWTAB }, m_iCurSel, {ptCursor.x, ptCursor.y}, 0};
				::SendMessage(GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM)&nmh);
			}
	...
	LRESULT OnLButtonDoubleClick(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/)
	{
		...
		NMCTCITEM nmh = {{ m_hWnd, (UINT_PTR)this->GetDlgCtrlID(), NM_DBLCLK }, nIndex, {ptCursor.x, ptCursor.y}, tchti.flags};
		if(FALSE == ::SendMessage(GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM)&nmh))
		{
			// returning FALSE let's us do our default handling
			if(nIndex >= 0)
	...
	LRESULT OnRButtonDown(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/)
	{
		...
		// returning TRUE tells us not to do our default handling
		NMCTCITEM nmh = {{ m_hWnd, (UINT_PTR)this->GetDlgCtrlID(), NM_RCLICK }, nIndex, {ptCursor.x, ptCursor.y}, tchti.flags};
		if(FALSE == ::SendMessage(GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM)&nmh))
		{
			// returning FALSE let's us do our default handling
			if(nIndex >= 0)
	...
	LRESULT OnRButtonDoubleClick(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/)
	{
		...
		// returning TRUE tells us not to do our default handling
		NMCTCITEM nmh = {{ m_hWnd, (UINT_PTR)this->GetDlgCtrlID(), NM_RDBLCLK }, nIndex, {ptCursor.x, ptCursor.y}, tchti.flags};
		if(FALSE == ::SendMessage(GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM)&nmh))
		{
			// returning FALSE let's us do our default handling
			if(nIndex >= 0)
	...
	LRESULT OnMButtonDown(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/)
	{
		...
		// returning TRUE tells us not to do our default handling
		NMCTCITEM nmh = {{ m_hWnd, (UINT_PTR)this->GetDlgCtrlID(), CTCN_MCLICK }, nIndex, {ptCursor.x, ptCursor.y}, tchti.flags};
		if(FALSE == ::SendMessage(GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM)&nmh))
		{
			// returning FALSE let's us do our default handling
			if( nIndex!=-1 )
	...
	LRESULT OnMButtonDoubleClick(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/)
	{
		...
		// returning TRUE tells us not to do our default handling
		NMCTCITEM nmh = {{ m_hWnd, (UINT_PTR)this->GetDlgCtrlID(), CTCN_MDBLCLK }, nIndex, {ptCursor.x, ptCursor.y}, tchti.flags};
		if(FALSE == ::SendMessage(GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM)&nmh))
		{
			// returning FALSE let's us do our default handling
			if( nIndex!=-1 )
		...
		if(wParam == GWL_STYLE)
		{
			...
			if(pStyles)
			{
					...
					(((pStyles->styleNew) & CTCS_SCROLL) == CTCS_SCROLL))
				{
					if(m_tooltip.IsWindow())
					{
						m_tooltip.AddTool(m_hWnd, IDS_TABCTRL_SCROLLRIGHT_TIP, &rcDefault, (UINT)ectcToolTip_ScrollRight);
						m_tooltip.AddTool(m_hWnd, IDS_TABCTRL_SCROLLLEFT_TIP,  &rcDefault, (UINT)ectcToolTip_ScrollLeft);
					}
					...
					(((pStyles->styleNew) & CTCS_NEWTABBUTTON) == CTCS_NEWTABBUTTON))
				{
					if(m_tooltip.IsWindow())
					{
						m_tooltip.AddTool(m_hWnd, IDS_TABCTRL_NEWTAB_TIPS, &rcDefault, (UINT)ectcToolTip_NewTab);
					}
				}
					...
					(((pStyles->styleNew) & CTCS_CLOSEBUTTON) == CTCS_CLOSEBUTTON))
				{
					if(m_tooltip.IsWindow())
					{
						m_tooltip.AddTool(m_hWnd, IDS_TABCTRL_CLOSE_TIP, &rcDefault, (UINT)ectcToolTip_Close);
					}
	...
	LRESULT OnGetToolTipInfo(int /*idCtrl*/, LPNMHDR pnmh, BOOL& /*bHandled*/)
	{
		...
		if(pToolTipInfo)
		{
			...
			if(id > 0 && id <= m_Items.GetCount())
			{
				...
				{
					pToolTipInfo->hinst = WTL::ModuleHelper::GetResourceInstance();
					pToolTipInfo->lpszText = (LPWSTR)IDS_TABCTRL_CLOSE_TIP;
				}
				else
				{
...
public:
	...
	int InsertItem(int nItem, TItem* pItem, bool bSelectItem = false)
	{
		...
		// Send notification
		NMCTCITEM nmh = {{ m_hWnd, (UINT_PTR)this->GetDlgCtrlID(), CTCN_INSERTITEM }, nItem, {-1,-1}, 0};
		::SendMessage(GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM)&nmh);
		// Select if first tab
		if( nNewCount==1 )
		{
	...
	BOOL DeleteItem(size_t nItem, bool bUpdateSelection = true, bool bNotify = true)
	{
		...
		if(bNotify)
		{
			...
			NMCTCITEM nmh = {{ m_hWnd, (UINT_PTR)this->GetDlgCtrlID(), CTCN_DELETEITEM }, (int)nItem, {-1,-1}, 0};
